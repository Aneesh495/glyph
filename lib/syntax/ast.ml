...
let value_binding ?(is_rec = false) ?(params = []) pat expr span =
  { vb_pat = pat; vb_params = params; vb_expr = expr; vb_rec = is_rec; vb_span = span }
...
(* Smart constructors for common expression shapes *)
...
let rec fold_expr f acc e =
  ...
  | Exp_record_update (base, fields) ->
      ...
      List.fold_left (fun a (_, e) -> fold_expr f a e) acc fields
  | Exp_field (e, _) | Exp_unop (_, e) | Exp_annotated (e, _) | Exp_index (e, _)
    ->
      fold_expr f acc e
