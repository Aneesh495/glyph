...
let remove_unreachable (f : func) : int =
  let before = Label.Map.cardinal f.blocks in
  ignore (prune_unreachable f);
  before - Label.Map.cardinal f.blocks

let prepare = ensure_cfg

let simplify_trivial_merges (f : func) : int =
  recompute_edges f;
  let merges = ref 0 in
  ...
  while !work do
    ...
    match
      Label.Map.fold
        (fun _ (b : block) acc ->
          ...
          | Some _ -> acc
          | None -> (
              match (b.preds, b.phis) with
              | [ pred ], [] -> (
                  match find_block f pred with
                  | Some pb
                    when (match pb.terminator with
                         | Jump (l, _) -> Label.equal l b.label
                         | _ -> false)
                         && List.length pb.succs = 1 ->
                      Some (pred, b.label)
                  | _ -> None)
              | _ -> None))
        f.blocks None
    ...
    | None -> ()
    | Some (pred, mid) -> (
        match (find_block f pred, find_block f mid) with
        | Some pb, Some mb ->
            pb.instrs <- pb.instrs @ mb.instrs;
            pb.terminator <- mb.terminator;
            f.blocks <- Label.Map.remove mid f.blocks;
            recompute_edges f;
            incr merges;
            work := true
        | _ -> ())
