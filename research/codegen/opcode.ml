...
let pp_opcode = pp_instr

let is_terminator = function
  | Op_jump | Op_switch | Op_tail_call | Op_tail_call_closure | Op_ret
  | Op_ret_void | Op_halt -> true
  | _ -> false

let encode_words (i : instr) : int32 list =
  let pack =
    Int32.(
      logor (of_int (op_to_int i.op land 0xff))
        (logor (shift_left (of_int (i.a land 0xff)) 8)
           (logor (shift_left (of_int (i.b land 0xff)) 16)
              (shift_left (of_int (i.c land 0xff)) 24))))
  in
  pack :: Int32.of_int i.a :: Int32.of_int i.b :: Int32.of_int i.c
  :: Int32.of_int (Array.length i.extra)
  :: Array.to_list (Array.map Int32.of_int i.extra)

let decode_words (words : int32 array) (idx : int) : instr * int =
  let op = op_of_int (Int32.to_int words.(idx) land 0xff) in
  let a = Int32.to_int words.(idx + 1) in
  let b = Int32.to_int words.(idx + 2) in
  let c = Int32.to_int words.(idx + 3) in
  let n = Int32.to_int words.(idx + 4) in
  let extra = Array.init n (fun i -> Int32.to_int words.(idx + 5 + i)) in
  ({ op; a; b; c; extra }, idx + 5 + n)
